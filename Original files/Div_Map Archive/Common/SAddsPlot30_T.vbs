...
If (WshFS.FolderExists(WshFS.GetFolder("S:\Workgroups\APC Power Delivery\Division Mapping Test\Common\"))) Then
	Set f = WshFS.GetFolder("S:\Workgroups\APC Power Delivery\Division Mapping Test\Common\")
	strInputPath02 = f.ShortPath & "\"
End If

pKey = WshShell.RegRead("HKCU\SOFTWARE\Autodesk\AutoCAD\R17.0\ACAD-5002:409\Profiles\AddsPlot_T\UserName")
...
Else
	...
	If (WshFS.FileExists(strInputPath01 & "Acad.exe")) Then
		...
		intReturn = WshShell.Run (strDQUOTE & strInputPath01 & "Acad.exe" & strDQUOTE & " /nologo /b " & strInputPath02 & "AddProfSAddsPlot_T.Scr", 1,False)
	Else
...
If (WshFS.FileExists(strInputPath01 & "Acad.exe")) Then
	'WScript.Echo "Calling Correct Version [Acad.exe]"
	intReturn = WshShell.Run (strDQUOTE & strInputPath01 & "Acad.exe" & strDQUOTE & " /nologo /p AddsPlot_T", 1,False)
Else
	'WScript.Echo "{Not} Calling Correct Version [Acad.exe]"
End If
