' Windows Scripting Host VB Script to Launch AddsPlot 3.0 for AutoCad Map 2004
'
' -----------------------------------------------------------------------------------------
'
...
strComment = strDQUOTE & "AddsPlot 3.0 Launcher" & strDQUOTE
StartTime = Time
DebugLevel = 0
WindowTitle = "AddsPlot 3.0 Launcher " & strVersionNum
Fnd_M = False
Fnd_N = False
...
End If
	...
	'Checks to see if AutoCAD is installed and installs it if not.
...
End If
	...
	'Runs command file to copy all need files to local computer.
...
End If
If (WshFS.FileExists(strInputPath02 & "Upd_SDIV_Map_Local.Cmd")) Then
	'WScript.Echo "Calling routine Upd_ADDS_Local.Cmd"
	intReturn = WshShell.Run (strInputPath02 & "Upd_SDIV_Map_Local.Cmd", 1, True)
Else
...
If (WshFS.FolderExists(WshFS.GetFolder("S:\Workgroups\APC Power Delivery\Division Mapping Test\Common\"))) Then
	Set f = WshFS.GetFolder("S:\Workgroups\APC Power Delivery\Division Mapping\Common\")
	strInputPath02 = f.ShortPath & "\"
End If

	'Checks if profile exists if it does not then launches AutoCAD and regerster the profile and set profile options.
pKey = WshShell.RegRead("HKCU\SOFTWARE\Autodesk\AutoCAD\R17.0\ACAD-5002:409\Profiles\AddsPlot\UserName")
If (pKey = "Information Technology, Southern Company") then
	'WScript.Echo "bKey Value = {" & pKey & "}"
Else
	'WScript.Echo "bKey Value <> {" & pKey & "}"
	If (WshFS.FileExists(strInputPath01 & "Acad.exe")) Then
		'WScript.Echo "Calling Correct Version [Acad.exe]"
		intReturn = WshShell.Run (strDQUOTE & strInputPath01 & "Acad.exe" & strDQUOTE & " /nologo /b " & strInputPath02 & "AddProfSAddsPlot.Scr", 1,False)
	Else
...
End If
	...
	'Launches AutoCAD in AddsPlot
If (WshFS.FileExists(strInputPath01 & "Acad.exe")) Then
	'WScript.Echo "Calling Correct Version [Acad.exe]"
	intReturn = WshShell.Run (strDQUOTE & strInputPath01 & "Acad.exe" & strDQUOTE & " /nologo /p AddsPlot", 1,False)
Else
	'WScript.Echo "{Not} Calling Correct Version [Acad.exe]"
End If
